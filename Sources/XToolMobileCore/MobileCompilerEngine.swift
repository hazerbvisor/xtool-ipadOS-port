...
/// Swift, Clang, Mach-O LLD and COFF LLD through a small stable C ABI.
///
...
public final class MobileCompilerEngine: MobileProjectCompiler, @unchecked Sendable {
    ...
    /// Executes LLD's Windows/COFF (`lld-link`) driver in-process.
    ///
    /// Pass ordinary lld-link arguments such as `/machine:x64`, `/entry:main`
    /// and `/out:<path>`. The native bridge supplies argv[0].
