...
public enum MobileProjectBuilder {
    public static func build(
        ...
        let appExtensions = manifest.extensions ?? []
        let totalProgress = targets.count + appExtensions.count + 2
        let sdk = try toolchain.mobileSwiftSDKConfiguration()
        guard engine.supportsMachOLLD else {
            throw MobileProjectBuildError.invalid("Mach-O linker is unavailable")
        }
        ...
        let cacheIdentity = [
            "module-cache-v2-validated-shims",
            project.root.path,
            toolchain.root.path,
            sdk.sdkURL.path,
            PreparedToolchain.expectedBundledRuntimeRevision,
            engine.location.path,
            String(describing: engineAttributes?[.modificationDate]),
            String(describing: engineAttributes?[.size]),
            String(describing: sdkAttributes?[.modificationDate]),
            manifestIdentity,
        ].joined(separator: "\n")
        let cache = try MobileModuleCache.directory(in: outputDirectory, identity: cacheIdentity)

        // Product and target names commonly match. Keep their filesystem namespaces
        // separate so LLD never receives a target directory as -o.
        ...
        func checked(_ result: MobileBuildResult, job: String, output: URL) throws {
            if !result.standardOutput.isEmpty {
                report(String(decoding: result.standardOutput, as: UTF8.self))
            }
            if !result.standardError.isEmpty {
                report(String(decoding: result.standardError, as: UTF8.self))
            }
            guard result.succeeded else {
                throw MobileProjectBuildError.failed(job, result.exitCode)
            }
        ...
        if !appExtensions.isEmpty {
            report("Embedded app extensions: " + appExtensions.map(\.name).joined(separator: ", "))
        }
        report("Build log: \(logURL.path)")
        report("Reusable module cache: \(cache.path)")
        report("Build progress: 0/\(totalProgress)")

        do {
            try stage("Preparing build")

            // Direct LLD invocation bypasses the Clang driver's automatic compiler-rt
            // linkage, including __isPlatformVersionAtLeast. Resolve it before compiling
            // so an incomplete runtime fails early.
            let compilerRuntime = try iOSCompilerRuntime(
                toolchain: toolchain,
                sdk: sdk,
                fileManager: fm
            )
            ...
            for (targetIndex, target) in targets.enumerated() {
                ...
                if !swiftSources.isEmpty {
                    ...
                    if target.parseAsLibrary ?? (target.name != manifest.executableTarget || !hasMainFile) {
                ...
                report("Build progress: \(targetIndex + 1)/\(totalProgress)")
            }
            ...
            guard !fm.fileExists(atPath: executable.path) else {
                throw MobileProjectBuildError.invalid(
                    "Link output path is already occupied: \(executable.path)"
                )
            }
            ...
            try checked(
                engine.runMachOLLD(
                    arguments: link,
                    diagnosticsURL: work.appendingPathComponent("link.stderr")
                ),
                job: "Link",
                output: executable
            )
            report("Build progress: \(targets.count + 1)/\(totalProgress)")
            ...
            guard header.count == 16,
                ...
                throw MobileProjectBuildError.invalid(
                    "Linker output is not an arm64 Mach-O executable"
                )
            }
            ...
            // Build each extension as a separate Mach-O executable. Keeping this out
            // of the host link prevents extension-only objects from leaking into the
            // application binary and preserves the pre-extension build path exactly.
            for (extensionIndex, appExtension) in appExtensions.enumerated() {
                try checkCancellation()
                try stage("Building app extension \(appExtension.name)")
                let extensionFiles = try MobileAppExtensionBuilder.build(
                ...
                report(
                    "Embedded \(appExtension.name).appex under PlugIns/"
                )
                report(
                    "Build progress: \(targets.count + 2 + extensionIndex)/\(totalProgress)"
                )
            }
            ...
            if manifest.reuseCompilerEngine == true {
                files.append(
                    MobileIPAFile(
                        sourceURL: engine.location,
                        relativePath: "Frameworks/libXToolCompilerEngine.dylib",
                        isExecutable: true
                    )
                )
            }
            if manifest.reuseBundledRuntime == true {
                guard let archive = Bundle.main.url(forResource: "MobileRuntime", withExtension: "tar") else {
                    throw MobileProjectBuildError.invalid(
                        "The installed app has no runtime archive to reuse"
                    )
                }
                files.append(
                    MobileIPAFile(sourceURL: archive, relativePath: "MobileRuntime.tar")
                )
            }
            ...
            if let path = manifest.infoPlist {
                guard let dict = try PropertyListSerialization.propertyList(
                    from: Data(contentsOf: input(path)),
                    format: nil
                ) as? [String: Any] else {
                    throw MobileProjectBuildError.invalid(
                        "Info.plist must contain a dictionary"
                    )
                }
                extraPlist = dict
            } else {
            ...
            try MobileIPAPackager.packageUnsignedIPA(
                ...
                configuration: MobileIPAConfiguration(
                    ...
                    shortVersion: manifest.shortVersion ?? "1.0",
                    buildVersion: manifest.buildVersion ?? "1",
                    minimumOSVersion: manifest.deploymentTarget
                ),
                additionalFiles: files,
                additionalInfoPlist: extraPlist,
                outputURL: ipa
            )
            report("SUCCESS: \(ipa.lastPathComponent)")
            report("Build progress: \(totalProgress)/\(totalProgress)")
            try MobileBuildLogRecovery.checkpoint(
                in: work,
                stage: currentStage,
                status: .succeeded
            )
    ...
    private static func iOSCompilerRuntime(
        ...
        for directory in resourceDirectories {
            let archive = directory
                .appendingPathComponent("lib/darwin/libclang_rt.ios.a")
